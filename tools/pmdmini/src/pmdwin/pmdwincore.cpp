...
#ifndef PMDMINI_HW_BACKEND
		/* On the real chip this fills the 256K ADPCM RAM through the
		 * data port one byte at a time (262144 busy-waited writes =
		 * ~7s of startup).  Every well-formed .M loads its own PCM
		 * data before playing it, so the fill is skipped in hardware
		 * mode.  (pc98snd patch.) */
