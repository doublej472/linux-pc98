...
/*
 * th05_game.cpp - Touhou 5: Mystic Square (Kaikidan)
 * Native Linux PC-98 Port using libpc98 SDK
 *
 * Gameplay:
 *   - Pinnacle PC-98 Danmaku Masterpiece with 4 Characters (Reimu, Marisa, Mima, Yuuka)
 *   - Dream Gauge system with full screen bullet canceling at 128 Dream
 *   - Dual Tag-Team Boss (Yuki & Mai) and Shinki (Creator of Makai) Multi-Wing Phases
 *   - Sub-pixel Grazing, Left Shift Focus Hitbox, OPNA audio effects
 */
...
struct TH05_State {
	float px, py;
	int character; /* 0 = Reimu, 1 = Marisa, 2 = Mima, 3 = Yuuka */
	int power;     /* 0..128 */
	int dream;     /* 0..128 */
	int graze;
	int score;
...
static TH05_State game;
static pc98_dat_t *dat_kaiki = NULL;
...
static void init_stage(int stg)
{
	...
	/* Load Authentic Stage Background .PI from KAIKI.DAT */
	char pi_name[32];
	snprintf(pi_name, sizeof(pi_name), "ST0%d.PI", (stg % 6 == 0) ? 6 : (stg % 6));
	size_t pi_sz = 0;
	uint8_t *pi_data = pc98_dat_read_file(dat_kaiki, pi_name, &pi_sz);
	if (pi_data) {
...
int main(int argc, char **argv)
{
	...
	static const char *th05_dats[] = { "KAIKI.DAT", "MAIN.DAT" };
	pc98_require_assets("Touhou 5: Mystic Square", "th05", th05_dats, 2);

	dat_kaiki = pc98_dat_open("th05", "KAIKI.DAT");
	dat_main  = pc98_dat_open("th05", "MAIN.DAT");

	printf("Starting Touhou 5: Mystic Square (Native PC-98 Port)...\n");
	...
	/* 16-color TH05 Palette */
	palette_set(0, 0, 0, 1);       /* Makai Space */
	...
	while (running && game.lives >= 0) {
		...
		if (!game.is_boss) {
			if (game.tick % 45 == 0 && game.tick < 1400) {
			...
			if (game.tick == 1500) {
				...
				game.boss_vx = 2.2f;
				game.boss_hp = 1400;
				game.boss_max_hp = 1400;
				game.boss_name = (game.stage == 1) ? "Sara" : ((game.stage == 2) ? "Luize" : ((game.stage == 3) ? "Alice" : "Shinki"));
				game.spell_name = (game.stage == 1) ? "Makai Gate" : ((game.stage == 2) ? "Curtain Fire" : ((game.stage == 3) ? "Puppeteer" : "Creator of Makai"));
				for (int e = 0; e < MAX_ENEMIES; e++) game.enemies[e].active = 0;

				/* Load Boss .CDG from KAIKI.DAT */
				char boss_file[32];
				snprintf(boss_file, sizeof(boss_file), "BOSS%d.CDG", (game.stage % 6 == 0) ? 6 : (game.stage % 6));
				size_t b_sz = 0;
				uint8_t *b_data = pc98_dat_read_file(dat_kaiki, boss_file, &b_sz);
				if (b_data) {
		...
		if (game.is_boss) {
			...
			/* Shinki Wing Danmaku Formations */
			if (game.tick % 20 == 0) {
				for (int a = 0; a < 12; a++) {
					float ang = (game.tick * 0.14f) + a * (2.0f * 3.14159f / 12.0f);
					spawn_enemy_bullet(game.boss_x, game.boss_y, cosf(ang) * 3.2f, sinf(ang) * 3.2f, 11);
				}
			}
		...
		for (int i = 0; i < MAX_PLAYER_SHOTS; i++) {
			if (game.p_shots[i].active) {
				...
				if (game.is_boss && game.p_shots[i].active &&
				    fabsf(game.p_shots[i].x - game.boss_x) < 32.0f &&
				    fabsf(game.p_shots[i].y - game.boss_y) < 26.0f) {
					game.boss_hp -= 5;
					game.p_shots[i].active = 0;
					game.score += 150;
					opna_ssg_tone(2000, 2, 1);
				}
			}
		}
		...
		pc98_draw_string_shadow(hud_x + 12, PLAYFIELD_Y + 16, "TOUHOU 5: MS", 14, 0);
		pc98_draw_string_shadow(hud_x + 12, PLAYFIELD_Y + 32, "MYSTIC SQUARE", 7, 0);
	...
	if (has_boss_cdg) pc98_cdg_free(&boss_cdg);
	pc98_dat_close(dat_kaiki);
	pc98_dat_close(dat_main);

	pc98_audio_close();
	graph_end();
	printf("Touhou 5 Exited Cleanly.\n");
	return 0;
}
